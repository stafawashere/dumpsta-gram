...
inside the engine's asyncio loop, below the bridge. One Swift call can trigger thirty
concurrent HTTP requests. See
[ADR-0001](../../docs/decisions/ADR-0001-async-core-sync-facade.md).

## The seam

`Bridge/` holds a typed Swift wrapper over the engine's `SyncClient`. Every method:

1. Runs on the Python queue.
2. Calls the engine.
3. Converts the result into a Swift value type.
4. Maps engine exceptions onto a Swift error enum.
...
dumpstagram.SyncClient
   which crosses into the engine's loop thread
```
...
```
engine listener writes into the event buffer
...
These are first-class engine states, not errors. The app surfaces them as something a
...
One engine client instance per account, held explicitly. There is no global client,
because the engine refuses to have one. See
...
engine feature. That is the payoff of the instance-scoped rule.
